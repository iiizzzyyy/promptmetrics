...
describe('BudgetService', () => {
  ...
  async function seedLogs(workspaceId: string, costs: number[], timestamps: number[]) {
    const db = getDb();
  ...
  describe('getWorkspaceSpend', () => {
    it('calculates correct spend for a workspace in a given month', async () => {
      ...
      await seedLogs('ws-a', [0.01, 0.02, 0.03], [startOfMonth, midMonth, startOfMonth + 1]);
      await seedLogs('ws-b', [0.5], [startOfMonth]);
    ...
    it('filters by month boundaries correctly', async () => {
      ...
      await seedLogs('ws-a', [0.01], [thisMonthStart]);
      await seedLogs('ws-a', [0.99], [prevMonthStart]);
  ...
  describe('checkBudget', () => {
    it('passes when spend is under budget', async () => {
      const db = getDb();
      await db.prepare('INSERT INTO config (key, value) VALUES (?, ?)').run('workspace_budget:under-ws', '100');
    ...
    it('throws when spend equals budget', async () => {
      const db = getDb();
      await db.prepare('INSERT INTO config (key, value) VALUES (?, ?)').run('workspace_budget:eq-ws', '0.05');

      const now = new Date();
      const yearMonth = `${now.getUTCFullYear()}-${String(now.getUTCMonth() + 1).padStart(2, '0')}`;
      const startOfMonth = Math.floor(new Date(`${yearMonth}-01T00:00:00Z`).getTime() / 1000);
      await seedLogs('eq-ws', [0.05], [startOfMonth]);
    ...
    it('throws when spend exceeds budget', async () => {
      const db = getDb();
      await db.prepare('INSERT INTO config (key, value) VALUES (?, ?)').run('workspace_budget:over-ws', '0.01');

      const now = new Date();
      const yearMonth = `${now.getUTCFullYear()}-${String(now.getUTCMonth() + 1).padStart(2, '0')}`;
      const startOfMonth = Math.floor(new Date(`${yearMonth}-01T00:00:00Z`).getTime() / 1000);
      await seedLogs('over-ws', [0.05], [startOfMonth]);
  ...
  describe('getWorkspaceBudget', () => {
    it('reads budget from config table', async () => {
      const db = getDb();
      await db.prepare('INSERT INTO config (key, value) VALUES (?, ?)').run('workspace_budget:custom-ws', '250.50');
    ...
    it('falls back to DEFAULT_MONTHLY_BUDGET_USD when config value is invalid (NaN)', async () => {
      const db = getDb();
      await db.prepare('INSERT INTO config (key, value) VALUES (?, ?)').run('workspace_budget:nan-ws', 'not-a-number');
